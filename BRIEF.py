...
def makeTestPattern(patch_width=9, nbits=256):
    ...
    compareX = np.random.randint(0, 81, size=256)
    compareY = np.random.randint(0, 81, size=256)
...
def computeBrief(im, gaussian_pyramid, locsDoG, k, levels,
    ...
    for y, x, level in locsDoG:
        if not ((x - 4) < 0  or (x + 5) > H or (y - 4) < 0 or (y + 5) > W):
            locs.append([y,x,level])
            # print (x-4,x+5, y-4,y+5, level)
            patch = gaussian_pyramid[x-4:x+5, y-4:y+5, level]
            patch = patch.flatten()
            # print(locsDoG[i], patch.shape)
            
            temp = []
            for k in range(256):
                if (patch[compareX[k]] < patch[compareY[k]]):
                    temp.append(1)
                else:
                    temp.append(0)
            desc.append(temp)
